...
async function getUntranslatedProducts(): Promise<Product[]> {
  ...
  const [productsRes, seoRes] = await Promise.all([
    ...
    fetch(
      `${AT_BASE}/Haya_SEO?fields[]=item_code&fields[]=meta_title_ar&filterByFormula=${encodeURIComponent('NOT({meta_title_ar}="")')}&maxRecords=200`,
      { headers: { Authorization: `Bearer ${API_KEY}` }, cache: 'no-store' }
    ),
  ])
...
async function translateBatch(products: Product[]): Promise<Translation[]> {
  const systemPrompt = `You are a professional Arabic medical translator for Hayat Supplies, a medical supplies company in Oman.
...
async function upsertSeoRecord(translation: Translation): Promise<void> {
  ...
  const checkRes   = await fetch(
    `${AT_BASE}/Haya_SEO?filterByFormula=${formula}&maxRecords=1`,
    { headers: { Authorization: `Bearer ${API_KEY}` }, cache: 'no-store' }
  )
