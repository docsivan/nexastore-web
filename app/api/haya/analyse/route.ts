...
const SYSTEM_PROMPT = 'You are Haya, the AI business intelligence engine for Hayat Supplies, a healthcare procurement company in Oman. Your insights are brief (2-3 sentences), specific, and actionable. No generic advice. Return ONLY valid JSON — no markdown, no explanation.'
...
export async function GET(req: NextRequest) {
    ...
    const packages = [
      {
        ...
        prompt:      `Hayat Supplies customer searches (last 30 days): ${topSearches.join(', ')}. Identify the top 2-3 products we should add or promote to fill this demand. Be specific with product names.${JSON_SUFFIX}`,
      },
      {
        pkg:         'hot_products',
      ...
      },
      {
        ...
        priority:    2,
        skip:        false,
        prompt:      `Signals: ${searches.length} searches, ${views.length} views, ${cartAdds.length} cart adds, ${abandons.length} abandons (last 30 days). Which category should Hayat Supplies prioritise for restocking and marketing this week?${JSON_SUFFIX}`,
      },
    ]
  ...
  if (orders.length >= 5) {
    ...
    const patternPrompt = `Analyze these 90-day patterns for Hayat Supplies and return the 3 most actionable business insights as a JSON array. Each object: {"insight_type":"pattern","insight":"...","action_required":"...","priority":1-5}\n\nData: ${JSON.stringify(patternData)}`
  ...
  if (conversations.length > 0) {
    ...
    const convoPrompt = `You are Haya's learning engine for Hayat Supplies Oman.
...
${transcriptBlock}`
    ...
    try {
      ...
      for (const insight of parsed.slice(0, 5)) {
        await fetch(`https://api.airtable.com/v0/${BASE_ID}/Haya_Insights`, {
          method: 'POST',
          headers: atHeaders(),
          body: JSON.stringify({
            fields: {
