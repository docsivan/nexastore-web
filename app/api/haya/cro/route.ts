...
import { runCROAgent, CROFix } from '@/lib/haya-agents'
...
async function fetchCROSignals() {
  ...
  const data = await atGet(
    `/Haya_CRO?filterByFormula=${formula}&maxRecords=100&sort[0][field]=session_count&sort[0][direction]=desc`
  )
...
async function rewriteProductDescription(itemCode: string): Promise<string | null> {
  ...
  const newDesc = await callSonnet(
    `Rewrite this product description to be more conversion-optimised for a medical supplies store in Oman. Focus on clinical benefits, compliance, and professional use. Max 200 words.\n\nProduct: ${name}\nCategory: ${category}\nCurrent description: ${description ?? '(none)'}`,
    'You are a medical product copywriter for Hayat Supplies, Oman. Write concise, professional, conversion-focused descriptions. Return only the description text, no labels.'
  )
