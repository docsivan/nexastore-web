...
import { handleReorderReminder, handleStockAlert, HayaInsight } from '@/lib/haya-actions'
...
async function fetchInsight(id: string): Promise<HayaInsight | null> {
  try {
    const res = await fetch(`${AT_BASE}/Haya_Insights/${id}`, {
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[POST /api/haya/approve]', error)
