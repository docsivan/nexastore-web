...
import { runCMOAgent, CMORecommendation } from '@/lib/haya-agents'
...
async function fetchRecentInsights() {
  ...
  const data = await atGet(
    `/Haya_Insights?filterByFormula=${formula}&maxRecords=10&sort[0][field]=created_at&sort[0][direction]=desc`
  )
  return (data.records ?? []).map((r: { fields: Record<string, unknown> }) => ({
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Write all 5 insights to Haya_Insights
    ...
    // (No Haya_Promotions record yet — that's created when owner approves via /api/haya/act)
