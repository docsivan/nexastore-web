...
async function fetchNewInsights(): Promise<{ id: string; insight: string; package: string; priority: number }[]> {
  try {
    ...
    const url = `${AT_BASE}/Haya_Insights?filterByFormula=${formula}&sort[0][field]=priority&sort[0][direction]=desc&maxRecords=3`
...
async function fetchCfoInsights(): Promise<{ id: string; insight_text: string }[]> {
  try {
    ...
    const url = `${AT_BASE}/Haya_Insights?filterByFormula=${formula}&sort[0][field]=priority&sort[0][direction]=asc&maxRecords=2`
    const res = await fetch(url, { headers: { Authorization: `Bearer ${API_KEY}` }, cache: 'no-store' })
    if (!res.ok) return []
    const data = await res.json()
    return (data.records ?? []).map((r: { id: string; fields: Record<string, unknown> }) => ({
...
async function acknowledgeInsights(ids: string[]) {
  for (const id of ids) {
    await fetch(`${AT_BASE}/Haya_Insights/${id}`, {
      method: 'PATCH',
      headers: atHeaders(),
...
export async function GET(req: NextRequest) {
  ...
  try {
    urgentAction = await callSonnet(
      ...
      'You are Haya, AI business intelligence for Hayat Supplies, Oman. Be concise, specific, and decisive.'
    )
  } catch {
  ...
  try {
    message = await callSonnet(
      ...
      'You are Haya, AI Business Owner of Hayat Supplies, Oman. Write concise, professional WhatsApp briefings.'
    )
  } catch {
